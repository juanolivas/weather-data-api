class LocationWeatherService
  ...
  def city_forecast(lat: nil, lon: nil)
    begin
      { success: false, data: {}, error: "Latitud y longitud son requeridas" } if lat.nil? || lon.nil?
      ...
      if data && !data[:error]
        ...
        { success: true, data: five_days_data }
      else
        { success: false, data: {}, error: forecast[:error] || "Error al obtener pronóstico" }
      end
    rescue => e
      { success: false, data: {}, error: e.message }
    end
